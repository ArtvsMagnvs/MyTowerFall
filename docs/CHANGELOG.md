...
### 1. Cadáver con screen wrap vertical (general)
`scripts/characters/Corpse.gd` ahora envuelve verticalmente igual que el player/monstruo.
Si el cuerpo (muerto por flecha, espada o stomp) cae al vacío, reaparece por arriba con la
misma X en vez de eliminarse. Así la flecha que lleva clavada sigue siendo recogible.
Sin mecánica nueva: misma regla que para entidades vivas (porque en TowerFall no hay
distinción entre "vivo cayendo" y "muerto cayendo").

### 2. Una flecha al revivir
`scripts/characters/PlayerBase.gd`: `AMMO_START 3 → 1` y `ammo` default `3 → 1`.
Tanto en el spawn inicial como en cada respawn. `AMMO_MAX` se queda en 5 (sigue siendo
el tope al recoger flechas del suelo).

### 3. Reducción global de velocidades (-15%)
Feeling de TowerFall: todo más "pesado" y menos arcade. Aplicado a TODOS los valores
de velocidad (sin tocar tiempos como cooldowns ni locks). Detalle por archivo:

**PlayerStats.tres** — caminar, saltar, dash, wall jump, stomp bounce, ledge slide:
| Stat | Antes | Después (-15%) |
|---|---|---|
| `walk_speed` | 78 | 66 |
| `jump_velocity` | -254 | -216 |
| `max_fall_speed` | 260 | 221 |
| `wall_jump_push` | 97.5 | 83 |
| `wall_jump_vertical` | -269 | -229 |
| `wall_slide_speed` | 60 | 51 |
| `dash_speed` | 240 | 204 |
| `stomp_bounce` | -180 | -153 |
| `stomp_bounce_held` | -254 | -216 |
| `ledge_slide_speed` | 30 | 26 |

**ArrowStats.tres** — proyectiles:
| Stat | Antes | Después (-15%) |
|---|---|---|
| `initial_speed` | 300 | 255 |
| `cone_speed` | 272 | 231 |
| `proj_gravity` | 200 | 170 |

**Monstruos** (scripts/monsters/*.gd + WarriorPlayer.gd):
| Stat | Antes | Después |
|---|---|---|
| Slime `PATROL_SPEED` | 15 | 13 |
| Slime `CHASE_SPEED` / `JUMP_VX` | 30 | 26 |
| Slime `JUMP_VY` | -50 | -43 |
| Slime `SLIME_GRAVITY` | 112 | 95 |
| Troll `PATROL_SPEED` | 20 | 17 |
| Troll `CHASE_SPEED` | 35 | 30 |
| Troll `ROCK_SPEED` | 160 | 136 |
| Troll `ROCK_GRAVITY` | 180 | 153 |
| Bat `FLY_SPEED` | 50 | 43 |
| Bat `DASH_SPEED` (igual que player) | 240 | 204 |
| Bat `UNSTUCK_SPEED` | 60 | 51 |
| Specter `FLY_SPEED` | 70 | 60 |
| Specter `BOLT_SPEED` | 160 | 136 |
| Specter `UNSTUCK_SPEED` | 40 | 34 |
| Warrior `CHARGE_SPEED` | 500 | 425 |

**NO tocado**: `gravity = 900` (afecta a todos los air-times proporcionalmente), ni
`dash_duration`, `dash_cooldown`, `dash_invuln_frames`, `coyote_time`, `jump_buffer_time`,
`wall_jump_lock`, `ledge_max_slide` (todos son tiempos o distancias, no velocidades).

### 4. Nueva secuencia de respawn (StoryMatch + VersusMatch)
Reemplaza el "viaje de gema → respawn → onda expansiva" antiguo. Flujo nuevo:

1. **Gema viaja al CUERPO** (0.9s, mismo arc que antes) — la gema ya no viaja al safe
   pos directamente, sino a donde está el cadáver flotando.
2. **Cuerpo se eleva + se ilumina** (0.3s) — placeholder geométrico (Y -5 px, modulate
   brighten). TODO pixel art: sprite/animación única de "cuerpo flotando".
3. **Aura dorada + gema crece** (1s total) — un Polygon2D dorado escala de 1.0→1.8 con
   alfa pulsante, y la gema crece de 1.0→1.7 al mismo tiempo. Al final del segundo 1,
   la gema se desvanece. TODO pixel art: sprite único de "carga de gema".
4. **Mini-carga con rayos eléctricos decorativos** (0.5s) — anillo de 6 líneas
   zigzagueantes que rota + pulso circular central amarillo. SIN hitbox (puramente
   visual). TODO pixel art: sprite único de "carga sobre cuerpo".
5. **Onda expansiva existente** (0.12s hitbox) — el player aparece con una pose
   "flotando" (Y -2 px, modulate brighten) durante la onda como placeholder. TODO
   pixel art: sprite/animación única de "durante la onda, todavía flotando".
6. **Blink de invuln** (1.5s on/off) — existente, sin cambios. El jugador ya no está
   flotando (vuelve a su pose normal con el parpadeo de invulnerabilidad).

Total: ~2.4s desde que la gema sale del HUD hasta que el jugador puede moverse.

### Implementación técnica
- `scripts/ui/StoryMatch.gd` y `scripts/ui/VersusMatch.gd` ahora comparten la misma
  secuencia (`_play_respawn_sequence`, `_on_gem_arrived`, `_play_mini_charge`,
  `_make_gem`). Código duplicado intencionalmente: la abstracción común no compensa
  para solo 2 llamadores.
- `scripts/characters/PlayerBase.respawn()` ahora llama a `_play_floating_vfx()` al
  final, que aplica el tween "flotando" sobre `_visual` durante 0.52s.
- `last_death_pos` se sigue usando como fallback si el corpse ya fue destruido.

### Tests
- 16/16 PASS, 0 regresiones. No hay test que dependa de `AMMO_START == 3` ni de las
  velocidades anteriores. El error "previously freed" en V083Test es pre-existente
  (test pasa con exit 0 de todas formas).

### Próximos pasos (Fase 0.9.x — pixel art)
Las 3 animaciones placeholder marcadas con TODO son trabajo de pixel art:
1. Sprite del "cuerpo flotando" (al llegar la gema).
2. Sprite del "aura/carga de la gema" (durante el segundo 1).
3. Sprite del "cuerpo durante la onda" (la pose flotando cuando aparece el player).
